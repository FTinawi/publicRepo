...
```{r,results='hold'}
CallCBrate <- sum(trxDenorm$ct_chgbk[trxDenorm$test_group == 1])/sum(trxDenorm$ct_rebill[trxDenorm$test_group == 1])
CallCBrate 
WebCBrate <- sum(trxDenorm$ct_chgbk[trxDenorm$test_group == 0])/sum(trxDenorm$ct_rebill[trxDenorm$test_group == 0])
WebCBrate
```

**Observation**: Chargeback rate for call-in users seems lower than for the web form users.  
Is this observation statistically significant?


We apply the z-test:
```{r}
res2 <- z.test(trxDenorm$revenues[trxDenorm$test_group == 1],trxDenorm$revenues[trxDenorm$test_group == 0],sigma.x = CallSdRev, sigma.y = WebSdRev,alternative = "greater",conf.level = 0.95)
res2
```